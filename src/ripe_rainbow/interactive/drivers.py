...
LOG_LEVELS = ("FINEST", "FINER", "FINE", "INFO", "WARNING", "SEVERE")

LOG_LEVELS_M = dict(
    FINEST="debug", FINER="debug", FINE="info", WARNING="warn", SEVERE="error"
)
...
class InteractiveDriver(object):
    ...
    def touch_diag(self, name, delta=1):
        if not hasattr(self.owner, "diag"):
            return
        if self.owner.diag == None:
            return
    ...
    def clear_storage(self):
        ...
        storage_protocol = self.instance.execute_script(
            "return !['data:'].includes(location.protocol);"
        )
...
class SeleniumDriver(InteractiveDriver):
    ...
    def browser(cls):
        ...
        if browser == "chrome":
            ...
            raise appier.OperationalError(message="Unknown browser '%s'" % browser)
    ...
    def click(self, element, ensure=True):
        ...
        try:
            ...
            self.owner.breadcrumbs.debug(
                'Element is not "clickable" because: %s (%s)'
    ...
    def scroll_to(self, element, sleep=None, position="center"):
        ...
        self.instance.execute_script(
            "arguments[0].scrollIntoView(%s);" % options, element
        )
    ...
    def close_tab(self, tab=None):
        ...
        if current_tab == tab:
            self.switch_tab(max(0, current_tab - 1))
        else:
    ...
    def instance(self):
        ...
        if (
            ...
            and cls._instance
            and hasattr(cls, "_options")
            and self.options == cls._options
        ):
        ...
        if self.browser == "chrome":
            ...
            raise appier.OperationalError(message="Unknown browser '%s'" % self.browser)
    ...
    def _selenium_options(self, browser=None):
        ...
        if not hasattr(self, "_selenium_options_%s" % browser):
            return None
    ...
    def _selenium_options_chrome(self):
        ...
        if self.mobile_emulation:
            ...
            emulation_options = dict(
                deviceMetrics=dict(
                    width=width, height=height, pixelRatio=self.pixel_ratio, touch=False
                )
            )
            if user_agent:
                emulation_options["userAgent"] = user_agent
    ...
    def _selenium_options_firefox(self):
        ...
        if self.mobile_emulation:
            self.owner.skip("Firefox can't run tests that require a mobile device")
    ...
    def _selenium_capabilities_chrome(self):
        ...
        capabilities["goog:loggingPrefs"] = dict(
            browser="ALL", driver="ALL", client="ALL", performance="ALL"
        )
    ...
    def _wait(self, timeout=None):
        ...
        if self.poll_frequency:
            kwargs["poll_frequency"] = self.poll_frequency
        if timeout == None:
    ...
    def _move_to(self, element, pivot="center"):
        ...
        if pivot == "center":
            return self._move_to_offset(
                element, x=int(element_width / 2), y=int(element_height / 2)
            )

        if pivot == "left-top":
            return self._move_to_offset(element, x=0, y=0)

        if pivot == "right-top":
            return self._move_to_offset(element, x=element_width - 1, y=0)

        if pivot == "right-bottom":
            return self._move_to_offset(
                element, x=element_width - 1, y=element_height - 1
            )

        if pivot == "left-bottom":
            return self._move_to_offset(element, x=0, y=element_height - 1)
    ...
    def _move_to_offset(self, element, x=0, y=0, safe=False):
        ...
        if safe:
            ...
            self._move_to_unsafe(actions, element, x, y)
        actions.perform()
    ...
    def _move_to_unsafe(self, actions, element, x=0, y=0, duration=5):
        ...
        instance.source.create_pointer_move(
            duration=duration, x=int(left), y=int(top), origin=element
        )
    ...
    def _flush_log(self, levels=LOG_LEVELS, browser=None):
        browser = browser or self.browser
        if not hasattr(self, "_flush_log_%s" % browser):
            return
    ...
    def _prettify_message(self, message):
        ...
        if event not in EVENT_STRINGIFIERS:
            return message
    ...
    def _gc_tabs(self):
        ...
        if len(self.instance.window_handles) < 1:
            return
    ...
    def __is_visible(self, element):
        ...
        if not self.secure and self.instance.execute_script("return window._entered"):
            return True
    ...
    def __install_listener(self, element):
        ...
        self.instance.execute_script("window._entered = false")
        self.instance.execute_script(
            "window._handler = function() { window._entered = true; };"
        )
        self.instance.execute_script(
            'arguments[0].addEventListener("mouseenter", window._handler, true);', element
        )
        self.instance.execute_script(
            'arguments[0].addEventListener("mouseover", window._handler, true);', element
        )
    ...
    def __uninstall_listener(self, element):
        ...
        self.instance.execute_script(
            'arguments[0].removeEventListener("mouseover", window._handler);', element
        )
        self.instance.execute_script(
            'arguments[0].removeEventListener("mouseenter", window._handler);', element
        )
...
class AppiumDriver(InteractiveDriver):
    ...
    def __init__(self, owner, **kwargs):
        ...
        self.activity = appier.conf(
            "APM_ACTIVITY", "com.platforme.ripe_robin.MainActivity"
        )
        self.app_path = appier.conf(
            "APM_APP_PATH",
            "/Users/gcc/ripe-robin-revamp/android/app/build/outputs/apk/debug/app-debug.apk",
        )
    ...
    def switch_context(self, context, *args, **kwargs):
        ...
        if context == "webview":
            ...
            webview_contexts = [
                context for context in self.contexts if context.startswith("WEBVIEW_")
            ]
            return self.instance.switch_to.context(webview_contexts[index])
        raise appier.OperationalError(message="Context not supported '%s'" % context)

    def count_context(self, context, *args, **kwargs):
        cls = self.__class__
        context_n = cls.CONTEXT_MAPPER.get(context, None)
        return len(
            [context for context in self.contexts if context.startswith(context_n)]
        )
