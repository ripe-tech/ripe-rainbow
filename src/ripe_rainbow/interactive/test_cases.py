...
class InteractiveTestCase(test_cases.TestCase):
    ...
    def _stacktrace(self, test, ctx=None):
        if not appier.conf("STACKTRACES", False, cast=bool):
            return
        if not self.driver:
            return
        ctx = ctx or {}
    ...
    def _screenshot(self, test, ctx=None):
        if not appier.conf("SCREENSHOTS", False, cast=bool):
            return
        if not self.driver:
            return
        ctx = ctx or {}
    ...
    def _store_logs(self, test, ctx=None):
        if not appier.conf("STORE_LOGS", True, cast=bool):
            return
        if not self.driver:
            return
        ctx = ctx or {}
        ...
        for memory_handler in memory_handlers:
            log_path = os.path.join(
                base_path, test_name + extra_s + ".%s.log" % memory_handler._name
            )
