...
class TestCase(appier.Observable):
    def __init__(self, *args, **kwargs):
        ...
        self.breadcrumbs = kwargs.get(
            "breadcrumbs", self.__class__._build_logger(name="ripe-rainbow-breadcrumbs")
        )
        self.browser_logger = kwargs.get(
            "browser_logger", self.__class__._build_logger(name="ripe-rainbow-browser")
        )

    @classmethod
    def _build_logger(
        cls, name="ripe-rainbow", level="INFO", silent=False, default=False
    ):
        ...
        if hasattr(TestCase, name_i):
            return getattr(TestCase, name_i)
        ...
        formatter = logging.Formatter(
            "%%(asctime)s [%s] [%%(levelname)s] %%(message)s" % name
        )
    ...
    def skip(self, message=None, reason=None):
        raise errors.SkipError(message=message, reason=reason or message)
