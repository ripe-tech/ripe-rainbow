...
class WaitsPart(parts.Part):
    ...
    def redirected_to(
        ...
        return self.until(
            lambda d: self.logic.at_url(
                url, params=params, fragment=fragment, starts_with=starts_with
            ),
            message=message
            or "Expecting the page to be '%s' but is '%s'"
    ...
    def tab_count(self, tab_count, message=None, timeout=None):
        return self.until(
            ...
            message=message
            or "Expecting the number of browser tabs to be '%s' but is '%s'"
            % (tab_count, self.driver.tab_count),
            timeout=timeout,
        )
    ...
    def visible(self, selector, text=None, ensure=True, message=None, timeout=None):
        return self.until(
            lambda d: self._get_element(
                selector, text=text, timeout=timeout, displayed=True, visible=ensure
            ),
            message=message or "Element '%s' not found or not visible" % selector,
            timeout=timeout,
        )

    def not_visible(self, selector, text=None, ensure=False, message=None, timeout=None):
        return self.until(
    ...
    def _get_element(
        self, selector, text=None, timeout=None, displayed=True, visible=True
    ):
        condition = None
        if text:
